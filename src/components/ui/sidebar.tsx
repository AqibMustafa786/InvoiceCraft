...
>(
  (
    ...
    const icon = childNodes.find(child => React.isValidElement(child) && (child.type as any).displayName?.includes('Icon'));
    const label = childNodes.find(child => !React.isValidElement(child) || !(child.type as any).displayName?.includes('Icon'));
