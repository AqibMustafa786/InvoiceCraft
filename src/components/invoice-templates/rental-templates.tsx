...
export const RentalTemplate1: React.FC<PageProps> = (props) => {
    const { invoice, pageItems, pageIndex, totalPages, balanceDue, currencySymbol, t, accentColor, total, subtotal, taxAmount, discountAmount } = props;
    ...
    return (
        <div className={`p-10 font-sans ${pageIndex < totalPages - 1 ? 'page-break-after' : ''}`} style={{ minHeight: '1056px', backgroundColor: props.backgroundColor, color: props.textColor }}>
            ...
            <section className="grid grid-cols-2 gap-8 text-sm mb-8">
                <div>
                    <p className="font-bold text-gray-500">{(t.renter || 'Renter').toUpperCase()}</p>
                    <p>{client.name}</p>
                ...
                <div className="text-right">
                    <p><span className="font-bold text-gray-500">{(t.date || 'Date').toUpperCase()}:</span> {safeFormat(invoice.invoiceDate, 'MMMM d, yyyy')}</p>
                    <p><span className="font-bold text-gray-500">{(t.dueDate || 'Due Date').toUpperCase()}:</span> {safeFormat(invoice.dueDate, 'MMMM d, yyyy')}</p>
            ...
            <main className="flex-grow mt-4">
                <table className="w-full text-left text-sm">
                    ...
                    <tbody>
                        {pageItems.map(item => (
                            <tr key={item.id} className="border-b"><td className="py-2">{item.name}</td><td className="py-2 text-right">{currencySymbol}{(item.quantity * item.unitPrice).toFixed(2)}</td></tr>
                        ))}
                    </tbody>
                </table>
            </main>
    ...
    return (
      <div className={`p-10 font-serif bg-white ${pageIndex < totalPages - 1 ? 'page-break-after' : ''}`} style={{ minHeight: '1056px', backgroundColor: props.backgroundColor, color: props.textColor }}>
        ...
        <section className="grid grid-cols-2 gap-8 mb-8 text-sm">
            <div>
                <p><strong>Billed For:</strong> {client.name}</p>
                <p className="whitespace-pre-line">{client.address}</p>
                <p>{client.phone}</p>
                <p>{client.email}</p>
            </div>
...
export const RentalTemplate4: React.FC<PageProps> = (props) => {
    ...
    return (
        <div className={`flex ${pageIndex < totalPages - 1 ? 'page-break-after' : ''}`} style={{ minHeight: '1056px', backgroundColor: props.backgroundColor, color: props.textColor }}>
            <div className="w-1/4 p-8 text-white" style={{backgroundColor: accentColor || '#be123c'}}>
                ...
                <div className="mt-10 text-xs space-y-4">
                    ...
                    <div><p className="opacity-70">DUE DATE</p><p>{safeFormat(invoice.dueDate, 'yyyy-MM-dd')}</p></div>
                </div>
            </div>
            <div className="w-3/4 p-10">
                <header className="text-right mb-10"><h2 className="text-2xl font-bold">{business.name}</h2><p className="text-xs whitespace-pre-line">{business.address}</p></header>
                <section className="mb-10 text-sm"><p><strong>To:</strong> {client.name}</p><p className="whitespace-pre-line">{client.address}</p></section>
                <RentalDetails invoice={invoice} t={t} />
                <main className="flex-grow mt-4">
                    <table className="w-full text-left text-sm">
...
export const RentalTemplate5: React.FC<PageProps> = (props) => {
    ...
    return (
        <div className={`p-10 font-serif ${pageIndex < totalPages - 1 ? 'page-break-after' : ''}`} style={{ minHeight: '1056px', backgroundColor: '#FDFBF7', color: '#5A4A42' }}>
            ...
            <h2 className="text-center text-xl mb-8">RENTAL {docTitle}</h2>
