...
export const MedicalTemplate1: React.FC<PageProps> = (props) => {
    ...
    const docTitle = t.medicalInvoice || 'Medical Invoice';
...
export const MedicalTemplate2: React.FC<PageProps> = (props) => {
    ...
    const { business, client } = invoice;
    const docTitle = t.medicalBillingStatement || 'Medical Billing Statement';
        ...
        <div className={`p-10 font-serif bg-gray-50 ${pageIndex < totalPages - 1 ? 'page-break-after' : ''}`} style={{ minHeight: '1056px', backgroundColor: props.backgroundColor, color: textColor }}>
            ...
            <main className="flex-grow mt-4">
                <table className="w-full text-left text-xs bg-white">
                    ...
                    <tbody>{pageItems.map(item => (<tr key={item.id} className="border-b"><td className="p-2">{safeFormat(invoice.invoiceDate, 'MM/dd/yyyy')}</td><td className="p-2">{item.name}</td><td className="p-2 text-right">{currencySymbol}{(item.quantity * item.unitPrice).toFixed(2)}</td></tr>))}</tbody>
                </table>
            </main>
            {pageIndex === totalPages - 1 && (
            <footer className="mt-auto pt-8">
                <div className="flex justify-end text-sm">
                    <div className="w-1/3">
                        ...
                        <p className="flex justify-between font-bold mt-2"><span>{t.patientResponsibility || 'Patient Responsibility'}:</span><span>{currencySymbol}{total.toFixed(2)}</span></p>
...
export const MedicalTemplate5: React.FC<PageProps> = (props) => {
    const { invoice, pageItems, pageIndex, totalPages, subtotal, taxAmount, discountAmount, total, balanceDue, currencySymbol, t, accentColor, textColor } = props;
    const { business, client } = invoice;
    const docTitle = (t.statementOfAccount || 'STATEMENT OF ACCOUNT');
    ...
    return (
        <div className={`p-8 font-serif ${pageIndex < totalPages - 1 ? 'page-break-after' : ''}`} style={{ minHeight: '1056px', backgroundColor: props.backgroundColor, color: textColor }}>
            ...
            <h2 className="text-center text-xl font-semibold mb-8">{docTitle}</h2>
