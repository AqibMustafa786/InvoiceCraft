                ...
                <footer className="mt-auto pt-8">
                    <div className="flex justify-between items-start">
                        ...
                        <div className="w-1/3 text-right text-xs space-y-2">
                             ...
                             {(invoice.amountPaid || 0) > 0 && <p className="grid grid-cols-2 mt-1 text-green-600"><span className="font-bold">{(t.amountPaid || 'PAID').toUpperCase()}</span> <span>-{currencySymbol}{(invoice.amountPaid || 0).toFixed(2)}</span></p>}
                             <p className="grid grid-cols-2 mt-1 pt-1 border-t font-bold text-base"><span className="font-bold">{(t.balanceDue || 'BALANCE DUE').toUpperCase()}</span> <span>{currencySymbol}{balanceDue.toFixed(2)}</span></p>
                        </div>
                    </div>
                </footer>
                )}
...
export const MedicalTemplate2: React.FC<PageProps> = (props) => {
    ...
    return (
        <div className={`p-10 font-serif bg-gray-50 ${pageIndex < totalPages - 1 ? 'page-break-after' : ''}`} style={{ minHeight: '1056px', backgroundColor: props.backgroundColor, color: textColor }}>
            ...
            <footer className="mt-auto pt-8">
                <div className="flex justify-end text-sm">
                    <div className="w-1/3">
                        ...
                        <p className="flex justify-between font-bold mt-2 pt-2 border-t"><span>{t.balanceDue || 'Balance Due'}:</span><span>{currencySymbol}{balanceDue.toFixed(2)}</span></p>
                    </div>
                </div>
            </footer>
            )}
...
export const MedicalTemplate3: React.FC<PageProps> = (props) => {
    const { invoice, pageItems, pageIndex, totalPages, total, balanceDue, t, currencySymbol, accentColor, textColor } = props;
    const { business, client } = invoice;
    const docTitle = (t.invoice || 'Invoice');
        ...
        <div className={`flex ${pageIndex < totalPages - 1 ? 'page-break-after' : ''}`} style={{ minHeight: '1056px', backgroundColor: props.backgroundColor, color: textColor }}>
            ...
            <div className="w-3/4 p-10">
                ...
                <footer className="mt-auto pt-8">
                    ...
                    {(invoice.amountPaid || 0) > 0 && <div className="flex justify-end text-lg font-bold text-green-600"><p><span>Paid: </span><span>-{currencySymbol}{(invoice.amountPaid || 0).toFixed(2)}</span></p></div>}
                    <div className="flex justify-end text-2xl font-bold mt-2"><p><span>Balance Due: </span><span>{currencySymbol}{balanceDue.toFixed(2)}</span></p></div>
                </footer>
                )}
            </div>
        </div>
            ...
            <footer className="mt-auto pt-8">
                <div className="flex justify-end text-right">
                    <div className="w-1/3 text-sm">
                        <p className="flex justify-between"><span>{t.total || 'Total'}:</span><span>{currencySymbol}{total.toFixed(2)}</span></p>
                        {discountAmount > 0 && <p className="flex justify-between text-red-600"><span>{t.discount || 'Discount'}:</span><span>-{currencySymbol}{discountAmount.toFixed(2)}</span></p>}
                        {invoice.summary.shippingCost > 0 && <p className="flex justify-between"><span>{t.shipping || 'Other Fees'}:</span><span>{currencySymbol}{invoice.summary.shippingCost.toFixed(2)}</span></p>}
                ...
                <div className="flex justify-end text-sm">
                    <div className="w-1/2">
                        ...
                        <p className="flex justify-between font-bold text-xl mt-2" style={{color: accentColor}}><span>{t.pleaseRemit || 'Please Remit'}: </span><span>{currencySymbol}{total.toFixed(2)}</span></p>
                         {(invoice.amountPaid || 0) > 0 && <p className="flex justify-between text-green-600 font-bold"><span>Paid:</span><span>-{currencySymbol}{(invoice.amountPaid || 0).toFixed(2)}</span></p>}
                         <p className="flex justify-between font-bold text-2xl mt-2" style={{color: accentColor}}><span>Balance Due: </span><span>{currencySymbol}{balanceDue.toFixed(2)}</span></p>
                    </div>
                </div>
            </footer>
            )}
