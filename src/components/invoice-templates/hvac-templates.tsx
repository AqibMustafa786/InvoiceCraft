...
export const HVACTemplate2: React.FC<PageProps> = (props) => {
    ...
    return (
        <div className={`p-8 bg-white font-sans text-gray-800 flex flex-col ${pageIndex < totalPages - 1 ? "page-break-after" : ""}`} style={{ fontFamily: 'Inter, sans-serif', fontSize: `9pt`, minHeight: '1056px', color: props.textColor }}>
    ...
    return (
        <div className={`p-10 bg-white font-sans ${pageIndex < totalPages - 1 ? 'page-break-after' : ''}`} style={{ minHeight: '1056px', backgroundColor: props.backgroundColor, color: props.textColor }}>
            ...
            <section className="grid grid-cols-2 gap-10 text-xs mb-8">
                <div><p className="font-bold text-gray-500 mb-1">{(t.billTo || 'BILL TO').toUpperCase()}</p><p>{client.name}<br/>{client.address}</p></div>
...
export const HVACTemplate8: React.FC<PageProps> = (props) => <HVACTemplate2 {...props} />;
export const HVACTemplate9: React.FC<PageProps> = (props) => <HVACTemplate3 {...props} />;
