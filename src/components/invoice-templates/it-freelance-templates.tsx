        ...
        <div className={`p-10 bg-white font-sans text-gray-800 flex flex-col ${pageIndex < totalPages - 1 ? "page-break-after" : ""}`} style={{ fontFamily: invoice.fontFamily, fontSize: `${invoice.fontSize}pt`, minHeight: '1056px', backgroundColor: props.backgroundColor, color: textColor }}>
            <header className="flex justify-between items-start pb-5 mb-5">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-xs" style={{color: accentColor}}>{business.email}</p>
                        {business.website && <p className="text-xs" style={{color: accentColor}}>{business.website}</p>}
            ...
            <section className="mb-8 p-4 bg-gray-50 rounded-md text-xs">
                ...
                <div className="grid grid-cols-[120px_1fr] gap-x-4 gap-y-1">
                    ...
                    <span className="text-gray-600">{t.estimationDate || 'ESTIMATION DATE'}:</span><span className="font-semibold">{safeFormat(invoice.invoiceDate, 'yyyy-MM-dd')}</span>
                </div>
            </section>
    ...
    return (
        <div className={`p-10 bg-gray-900 text-gray-200 font-['Roboto_Mono',_monospace] flex flex-col ${pageIndex < totalPages - 1 ? "page-break-after" : ""}`} style={{ minHeight: '1056px', backgroundColor: props.backgroundColor, color: props.textColor }}>
            ...
            <section className="grid grid-cols-2 gap-8 mb-8 text-xs">
                <div><p className="font-bold text-gray-500 mb-1">{(t.projectFor || 'PROJECT FOR').toUpperCase()}:</p><p className="font-medium">{client.name}</p><p className="text-gray-400">{client.address}</p>{client.shippingAddress && <p className="text-gray-400 mt-2"><span className="font-bold">Ship To:</span><br/>{client.shippingAddress}</p>}</div>
                <div className="text-right">
    ...
    return (
        <div className={`p-12 bg-white font-['Inter',_sans-serif] text-gray-800 flex flex-col ${pageIndex < totalPages - 1 ? "page-break-after" : ""}`} style={{ minHeight: '1056px', backgroundColor: props.backgroundColor, color: textColor }}>
            ...
            <section className="grid grid-cols-4 gap-4 mb-10 text-xs">
                ...
                <div className="text-right"><p className="font-bold text-gray-500 mb-1">{t.dueDate || 'Due Date'}</p><p>{safeFormat(invoice.dueDate, 'yyyy-MM-dd')}</p></div>
            </section>
