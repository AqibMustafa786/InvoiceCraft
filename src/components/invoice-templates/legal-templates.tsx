                ...
                <div className="flex justify-end text-xs">
                    <div className="w-1/3">
                        ...
                        <div className="flex justify-between p-1 mt-1 border-t-2 border-black font-bold"><span className="">{t.total || 'Total'}</span><span>{currencySymbol}{total.toFixed(2)}</span></div>
                         {(invoice.amountPaid || 0) > 0 && <div className="flex justify-between p-1 text-green-600"><span className="font-bold">Paid</span><span>-{currencySymbol}{(invoice.amountPaid || 0).toFixed(2)}</span></div>}
                        <div className="flex justify-between p-1 mt-1 border-t-2 border-black font-bold text-sm"><span className="">Balance Due</span><span>{currencySymbol}{balanceDue.toFixed(2)}</span></div>
                    </div>
                </div>
                ...
                <footer className="mt-auto pt-8">
                    <div className="flex justify-end text-sm">
                        <div className="w-2/5 space-y-1">
                            ...
                            <p className="flex justify-between font-bold text-base mt-2 pt-2 border-t border-gray-400"><span>{t.balanceDue || 'Balance Due'}:</span><span>{currencySymbol}{balanceDue.toFixed(2)}</span></p>
                        </div>
                    </div>
                    <div className="flex justify-between mt-8">
                      <SignatureDisplay signature={business.ownerSignature} label={t.authorizedSignature || 'Authorized Signature'} />
    ...
    return (
        <div className={`p-10 font-serif bg-white ${pageIndex < totalPages - 1 ? 'page-break-after' : ''}`} style={{ minHeight: '1056px', backgroundColor: props.backgroundColor, color: textColor }}>
            ...
            <section className="grid grid-cols-2 gap-8 mb-8 text-sm">
                ...
                <div className="text-right"><p><strong>{docTitle} #:</strong> {invoice.invoiceNumber}</p><p><strong>{t.date || 'DATE'}:</strong> {safeFormat(invoice.invoiceDate, 'MMMM d, yyyy')}</p></div>
            </section>
            <CategorySpecificDetails invoice={invoice} t={t} />
            <main className="flex-grow mt-4">
                <table className="w-full text-left text-sm">
        ...
        <div className={`flex ${pageIndex < totalPages - 1 ? 'page-break-after' : ''}`} style={{ minHeight: '1056px', backgroundColor: props.backgroundColor, color: textColor }}>
            ...
            <div className="w-3/4 p-10">
                ...
                <section className="text-sm mb-10">
                    ...
                    <p><strong>{docTitle} #:</strong> {invoice.invoiceNumber} | <strong>{t.date || 'Date'}:</strong> {safeFormat(invoice.invoiceDate, 'yyyy-MM-dd')}</p>
                </section>
                <CategorySpecificDetails invoice={invoice} t={t} />
                <main className="flex-grow mt-4">
                    <table className="w-full text-left text-sm">
                    ...
                    <div className="flex justify-end">
                      <div className="w-1/2 text-right">
                        ...
                        <div className="flex justify-between text-2xl font-bold mt-2 pt-2 border-t"><span>{t.totalDue || 'Total Due'}:</span><span>{currencySymbol}{total.toFixed(2)}</span></div>
                         {(invoice.amountPaid || 0) > 0 && <div className="flex justify-between text-green-600 font-bold"><span>Paid:</span><span>-{currencySymbol}{(invoice.amountPaid || 0).toFixed(2)}</span></div>}
                        <div className="flex justify-between text-2xl font-bold mt-2 pt-2 border-t-2"><span>Balance Due:</span><span>{currencySymbol}{balanceDue.toFixed(2)}</span></div>
                      </div>
                    </div>
                    <div className="flex justify-between mt-8">
                      <SignatureDisplay signature={business.ownerSignature} label={t.authorizedSignature || 'Authorized Signature'} />
