...
export default function BrandedLoginPage({ params }: { params: { slug: string } }) {
    const { slug } = params;
    ...
    useEffect(() => {
        const fetchCompany = async () => {
            ...
            try {
                ...
                // Filter by enabled in code to avoid composite index requirement
                ...
                setCompany(data);
                // Set companyIdHint for bootstrapUser
                sessionStorage.setItem('companyIdHint', data.id);
            } catch (err) {
                console.error("Error fetching branded portal:", err);
                router.push('/login');
            } finally {
                setIsLoading(false);
            }
    ...
    const onSubmit = async (data: LoginFormValues) => {
        ...
        try {
            ...
            toast({ title: "Login Successful", description: `Welcome back to ${company?.name}` });
    ...
    const getRadiusClass = (radius: string) => {
        switch (radius) {
            ...
            case 'full': return 'rounded-[2rem]'; // Full round for card is usually just very rounded
